...
class RoIRotate(object):
	...
	def roi_rotate_tensor_pad(self, feature_map, transform_matrixs, box_masks, box_widths, target_height=16):
		with tf.variable_scope("RoIrotate"):
			max_width = box_widths[tf.argmax(box_widths, 0, output_type=tf.int32)]
			# box_widths = tf.cast(box_widths, tf.float32)
			tile_feature_maps = []
			...
			def body(pad_rois, i):
				...
				roi = tf.image.crop_to_bounding_box(_affine_feature_map, 0, 0, target_height, target_height)
				pad_roi = tf.image.pad_to_bounding_box(roi, 0, 0, target_height, target_height)
			...
			# concat the left & right roi feature
			pad_rois_L = pad_rois[::2, :, :, :]
			pad_rois_R = pad_rois[1::2, :, :, :]
			pad_rois = tf.concat([pad_rois_L, pad_rois_R], axis=1)
