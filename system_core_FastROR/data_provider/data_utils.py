...
def polygon_sort(poly):
    point_len = []
    for i in range(4):
        point_len.append(dst_cal(poly[i][0], poly[i][1], 640, 0))
...
def check_is_horizon(poly):
    """
    Since FOTS can not deal with vertical blister area
    so we check it
    """
...
def check_and_validate_polys(polys, tags, labels, xxx_todo_changeme):
    '''
    check so that the  poly is in the same direction,
    ...
    for poly, tag, label in zip(polys, tags, labels):
        ...
        #     # print("vertical area")
        #     continue
        # front side rotate the poly 180 degree
        if count == 1 :
            poly = poly[(2, 3, 0, 1), :]
...
def crop_area(im, polys, tags, crop_background=False, max_tries=50):
    ...
    # ensure the cropped area not across a blister
    h_axis = np.where(h_array == 0)[0]
    w_axis = np.where(w_array == 0)[0]
    if len(h_axis) == 0 or len(w_axis) == 0:
        return im, polys, tags, np.array(len(polys))
...
def restore_rectangle_rbox(origin, geometry):
    ...
    if origin_0.shape[0] > 0:
        ...
        # do the affine transform of the rotate
...
def generate_rbox(im_size, polys, tags):
    ...
    for poly_idx, poly_tag in enumerate(zip(polys, tags)):
        ...
        # print("\nprepare_one_img_cost:" + str(time.time() - prepare_one_img_time))
    return training_mask, rectangles


def generate_maps(im_size, polys, tags):
    h, w = im_size
    poly_mask = np.zeros((h, w), dtype=np.uint8)
    score_map = np.zeros((h, w), dtype=np.uint8)
    geo_map = np.zeros((h, w, 5), dtype=np.float32)
    ...
    for poly_idx, poly_tag in enumerate(zip(polys, tags)):
        ...
        rectange = rectangle_from_parallelogram(parallelogram)
        rectange, rotate_angle = sort_rectangle(rectange)
        # rectange = polygon_sort(rectange)
        rectangles.append(rectange.flatten())
        p0_rect, p1_rect, p2_rect, p3_rect = rectange
        # prepare_one_img_time = time.time()
        for y, x in xy_in_poly:
...
def get_project_matrix_and_width(polyses, tags, target_height=8.0):
    project_matrixes = []
    box_widths = []

    for i in range(polyses.shape[0]):
        x1, y1, x2, y2, x3, y3, x4, y4 = polyses[i] / 4
        # x1, y1, x2, y2, x3, y3, x4, y4 = polyses[i]
...
def get_project_matrix(polyses, tags, target_height=16):
    project_matrixes = []
    box_widths = []
